...
class HumanityClient {
  ...
  log(msg, type = "info") {
    const prefix = `Ví ${this.accountIndex + 1}] [Wallet: ${this.queryId}]`;
    switch (type) {
      case "success":
        console.log(`[${prefix}] [✓] ${msg}`.green);
        break;
      case "custom":
        console.log(`[${prefix}] [*] ${msg}`.magenta);
        break;
      case "error":
  ...
  async claimReward(privateKey, address) {
    try {
      ...
      this.log(`Gas Price: ${this.web3.utils.fromWei(gasPrice, "gwei")} gwei | Gas Limit: ${gasLimit} | Nonce: ${tx.nonce}`, "custom");
  ...
  async runAccount() {
    const privateKey = this.queryId.startsWith("0x") ? this.queryId : `0x${this.queryId}`;
    ...
    console.log(`Ví ${this.accountIndex + 1} | ${address.green} | ip: ${proxyIP} | Bắt đầu sau ${timesleep} giây...==========`);
...
async function runWorker(workerData) {
  ...
  const to = new HumanityClient(queryId, accountIndex, proxy);
  try {
...
async function main() {
  ...
  console.log("Humanity Protocol Bot".yellow);
  console.log("https://t.me/mitomchanel".yellow);
